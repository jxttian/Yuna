...
//初始化面板数据
    Dates.initDate = function () {
        ...
        if(Dates.options.initTime){
            var hms = Dates.options.initTime.split(":");
            if (hms.length == 3) {
                ymd[3] = hms[0];
                ymd[4] = hms[1];
                ymd[5] = hms[2];
...
//        }
//    };
    Dates.creation = function (ymd, hide, date) {
        var S = Dates.query, hms = Dates.hmsin;
        var getDates = date ? Dates.parse(ymd, [date.getHours(), date.getMinutes(), date.getSeconds()]) : Dates.parse(ymd, [hms[0].value, hms[1].value, hms[2].value]);
        Dates.elem[as.elemv] = getDates;
        if (!hide) {
            Dates.close();
            typeof Dates.options.choose === 'function' && Dates.options.choose(getDates);
...
//事件
    Dates.events = function () {
        ...
        Dates.on(as.otoday, 'click', function () {
            var now = new Date();
            Dates.creation([now.getFullYear(), now.getMonth() + 1, now.getDate()], undefined, now);
        });
